...
export class WebSocketApi {
  ...
  public send(method: string, param?: any) {
    services.rest.RestApi.logAction({action: 'отправка send websocket', data: {method, param}})
  ...
  private switcher(wsString: WSS.massage) {
    switch (wsString.type) {
      default:
        services.rest.RestApi.logAction({action: 'нет функции для switcher', data: wsString.data})
        break
    }
  }
}